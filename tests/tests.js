...
beforeEach(() => {
  MANAGER = require('../task/app')();
});

function compareUnits(first, second) {
  if (second.speed - first.speed) return second.speed - first.speed;
...
describe('Zero tests', () => {
  ...
  it('getCommander should be a function with 3 arguments', () =>
    assertFunctionAndArgsCount(MANAGER.getCommander, 3));

  it('getArmyUnit should be a function with 1 argument', () =>
    assertFunctionAndArgsCount(MANAGER.getArmyUnit, 1));

  it('getSpell should be a function with 3 arguments', () =>
    assertFunctionAndArgsCount(MANAGER.getSpell, 3));

  it('addCommanders should be a function', () =>
    assertFunction(MANAGER.addCommanders));

  it('addArmyUnit to should be a function with 2 arguments', () =>
    assertFunctionAndArgsCount(MANAGER.addArmyUnitTo, 2));

  it('addSpellsTo to should be a function', () =>
    assertFunction(MANAGER.addSpellsTo));

  it('findCommanders should be a function with 1 argument', () =>
    assertFunctionAndArgsCount(MANAGER.findCommanders, 1));

  it('findArmyUnitById should be a function with 1 argument', () =>
    assertFunctionAndArgsCount(MANAGER.findArmyUnitById, 1));

  it('findArmyUnits should be a function with 1 argument', () =>
    assertFunctionAndArgsCount(MANAGER.findArmyUnits, 1));

  it('spellcast should be a function with 3 arguments', () =>
    assertFunctionAndArgsCount(MANAGER.spellcast, 3));

  it('battle should be a function with 2 arguments', () =>
    assertFunctionAndArgsCount(MANAGER.battle, 2));
});
  ...
  describe('Commanders and related methods', () => {
    describe('Valid', () => {
      ...
      it('should add all passed commanders', () => {
        ...
        commanders.forEach(function (c) {
          expect(
            MANAGER.findCommanders({ name: c.name, alignment: c.alignment })[0]
          ).to.equal(c);
        });
      });
      ...
      it('should add commander to battlemanager and make it retrievable via find', () => {
        ...
        MANAGER.addCommanders(koceto).addCommanders(cyki);
      ...
      it('should find commanders by name and/or alignment correctly', () => {
        ...
        function assertAlignmentSearch(alignment) {
          const names = MANAGER.findCommanders({ alignment })
              .map((c) => c.name)
              .sort(compareNames),
            originalames = commanders
              .filter((c) => c.alignment === alignment)
              .map((c) => c.name)
              .sort(compareNames);
        ...
        const darthVader = MANAGER.findCommanders({
          name: 'Darth Vader',
          alignment: 'evil',
        })[0];

        expect(darthVader).to.equal(
          commanders.find((c) => c.name === 'Darth Vader')
        );

        expect(
          MANAGER.findCommanders({ name: 'Homer', alignment: 'evil' }).length
        ).to.equal(0);
      });
    });
    ...
    describe('Invalid', () => {
      it('should throw with correct message for bad commander name', () => {
        const addCommanderWithInvalidNameLength = () =>
          MANAGER.getCommander('a', 'good', 13);

        expect(addCommanderWithInvalidNameLength).to.throw(
          Error,
          ERROR_MESSAGES.INVALID_NAME_LENGTH
        );

        const addCommanderWithInvalidSymbolsInName = () =>
          MANAGER.getCommander('_x;@', 'evil', 15);

        expect(addCommanderWithInvalidSymbolsInName).to.throw(
          Error,
          ERROR_MESSAGES.INVALID_NAME_SYMBOLS
        );
      });

      it('should throw with correct message for bad alignment', () => {
        const addCommanderWithBadAlignment = () =>
          MANAGER.getCommander('Hans', 'pizza', 33);

        expect(addCommanderWithBadAlignment).to.throw(
          Error,
          ERROR_MESSAGES.INVALID_ALIGNMENT
        );
      });
      ...
      it('should throw with correct message for bad mana', () => {
        ...
        addCommandersWithBadMana.forEach((c) =>
          expect(c).to.throw(Error, ERROR_MESSAGES.INVALID_MANA)
        );
      });
    });
  });
  ...
  describe('Army units and related methods', () => {
    describe('Valid', () => {
      ...
      it('should add ArmyUnit to commander correctly', () => {
        ...
        MANAGER.addCommanders(commander).addArmyUnitTo(
          commander.name,
          javaScriptDevs
        );
      ...
      it('should correctly add multiple units to commanders', () => {
        ...
        const armyUnits = {
          ...
          codeMonkeys: MANAGER.getArmyUnit({
            name: 'Code Monkeys',
            speed: 5,
            damage: 5,
            health: 5,
        ...
        MANAGER.addArmyUnitTo('Cyki', armyUnits.pikemen)
      ...
      it('find army units should work with different options variaties', () => {
        ...
        MANAGER.addArmyUnitTo('Tinky Winky', armyUnits.dwarfs)
        ...
        const microsoftMages = MANAGER.findArmyUnits({
          name: 'Mages',
          alignment: 'evil',
        });
      ...
      it('findArmyUnits should return units sorted by speed descending and by name ascending', () => {
        ...
        const sortedArmyUnits = Object.keys(armyUnits)
          .map((k) => armyUnits[k])
          .sort(compareUnits);
  ...
  describe('Spells', () => {
    describe('Valid', () => {
      it('should create spells with correct properties', () => {
        const spells = {
          confusion: MANAGER.getSpell(
            'Confusion',
            15,
            (target) => (target.speed -= 5)
          ),
          magicArrow: MANAGER.getSpell(
            'Magic Arrow',
            5,
            (target) =>
              (target.count -=
                (target.health * target.count - 200) / target.health)
          ),
          polymorph: MANAGER.getSpell('Polymorph', 20, (target) => {
            target.speed = 0;
            target.damage = 0;
            target.health *= 2;
          }),
      ...
      it('should correctly add spells to commanders', () => {
        ...
        const spells = {
          psionicStorm: MANAGER.getSpell(
            'Psionic Storm',
            15,
            (target) =>
              (target.count -=
                (target.health * target.count - 400) / target.health)
          ),
          transfuse: MANAGER.getSpell(
            'Transfuse',
            10,
            (target) => (target.health += 5)
          ),
        };

        MANAGER.addCommanders(mengsk, kerrigan).addSpellsTo(
          'Sarah Kerrigan',
          spells.psionicStorm,
          spells.transfuse
        );
    ...
    describe('Invalid', () => {
      it('should throw when spell is created with invalid parameters', () => {
        const getSpellInvalidName = () =>
            MANAGER.getSpell(';;;;;', 15, (target) => (target.speed += 1)),
          getSpellInvalidManaCost = () =>
            MANAGER.getSpell('dsfdsf', -5, (target) => (target.speed += 1)),
          getSpellInvalidEffect = () => MANAGER.getSpell('dfsf', 50, {});

        expect(getSpellInvalidEffect).to.throw(
          Error,
          ERROR_MESSAGES.INVALID_EFFECT,
          'Should throw for invalid effect'
        );
        expect(getSpellInvalidName).to.throw(
          Error,
          ERROR_MESSAGES.INVALID_NAME_SYMBOLS,
          'Should throw for invalid name'
        );
        expect(getSpellInvalidManaCost).to.throw(
          Error,
          ERROR_MESSAGES.INVALID_MANA,
          'Should throw for invalid mana cost'
        );
      });
      ...
      it('should throw for addition of objects that are not Spell-like', () => {
        ...
        const addSpellWithBadName = () =>
          MANAGER.addSpellsTo('Mage', validSpell, {
            name: 'Magic ^.^',
            manaCost: 15,
            effect: (target) => {},
          });
        expect(addSpellWithBadName).to.throw(
          Error,
          ERROR_MESSAGES.INVALID_SPELL_OBJECT
        );

        const addSpellWithBadManaCost = () =>
          MANAGER.addSpellsTo('Mage', validSpell, {
            name: 'Magic',
            manaCost: -1,
            effect: (target) => {},
          });
        expect(addSpellWithBadManaCost).to.throw(
          Error,
          ERROR_MESSAGES.INVALID_SPELL_OBJECT
        );

        const addSpellWithBadEffect = () =>
          MANAGER.addSpellsTo('Mage', validSpell, {
            name: 'Magic',
            manaCost: 15,
            effect: null,
          });
        expect(addSpellWithBadEffect).to.throw(
          Error,
          ERROR_MESSAGES.INVALID_SPELL_OBJECT
        );
    ...
    describe('spellcast', () => {
      describe('Valid', () => {
        it('should apply spell effect with spellcast', () => {
          ...
          const spells = {
            psionicStorm: MANAGER.getSpell('Psionic Storm', 15, (target) => {
              target.count -=
                (target.health * target.count - 400) / target.health;
              psiStormEffectExecuted = true;
            }),
            transfuse: MANAGER.getSpell('Transfuse', 10, (target) => {
              target.health += 5;
          ...
          MANAGER.addCommanders(mengsk, kerrigan)
            .addSpellsTo(
              'Sarah Kerrigan',
              spells.psionicStorm,
              spells.transfuse
        ...
        it('should decrease the casters mana when spellcasting', () => {
          ...
          MANAGER.spellcast('Sarah Kerrigan', 'Transfuse', units.zerglings.id);

          expect(kerrigan.mana).to.equal(
            50 - spells.psionicStorm.manaCost - spells.transfuse.manaCost
          );
        });
      ...
      describe('Invalid', () => {
        it('shoud throw when commander is not found', () => {
          const spellcastWithInvalidCommanderName = () =>
            MANAGER.spellcast('George', 'Some spell', 13);

          expect(spellcastWithInvalidCommanderName).to.throw(
            Error,
            "Can't cast with non-existant commander George!"
          );
        });
        ...
        it('should throw when spell is not found', () => {
          ...
          const spellcastWithUnknownSpell = () =>
            MANAGER.spellcast('Cyki', 'Write C#', 0);

          expect(spellcastWithUnknownSpell).to.throw(
            Error,
            `Cyki doesn't know Write C#`
          );
        });
        ...
        it('should throw when not enough mana', () => {
          ...
          MANAGER.addCommanders(kerrigan)
            .addSpellsTo('Sarah Kerrigan', transfuse)
            .addArmyUnitTo('Sarah Kerrigan', zerglings);

          const spellcastWithNoMana = () =>
            MANAGER.spellcast('Sarah Kerrigan', 'Transfuse', zerglings.id);

          expect(spellcastWithNoMana).to.throw(
            Error,
            ERROR_MESSAGES.NOT_ENOUGH_MANA
          );
        });
      });
    });
    ...
    describe('battle', () => {
      describe('Valid', () => {
        it('should correctly decrease defender count', () => {
          ...
          const countAfterBattle = Math.ceil(
            (programmers.health * programmers.count -
              penguins.damage * penguins.count) /
              programmers.health
          );
        ...
        it('should deal lower damage with attacked units', () => {
          ...
          const countAfterSecondBattle = Math.ceil(
            (penguins.health * penguins.count -
              programmers.damage * programmers.count) /
              penguins.health
          );
      ...
      describe('Invalid', () => {
        it('should throw when attacker or defender is not ArmyUnit-like', () => {
          ...
          const simulateInvalidAttackerBattle = () =>
            MANAGER.battle({ damage: 10, count: 'gosho' }, programmers);
          expect(simulateInvalidAttackerBattle).to.throw(
            Error,
            ERROR_MESSAGES.INVALID_BATTLE_PARTICIPANT
          );

          const simulateInvalidDefenderBattle = () =>
            MANAGER.battle(programmers, { damage: 10, count: 'gosho' });
          expect(simulateInvalidDefenderBattle).to.throw(
            Error,
            ERROR_MESSAGES.INVALID_BATTLE_PARTICIPANT
          );
        });
